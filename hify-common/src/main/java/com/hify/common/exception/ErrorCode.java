...
public enum ErrorCode {
    ...
    /** 调用超时：连接、首字节或流间隔超过对应的超时闸（CLAUDE.md 6.1），首字节之后不重试 */
    PROVIDER_TIMEOUT(2000, "模型服务响应超时，请稍后重试"),

    /** 认证失败：provider 的 API Key 无效或已过期，401/403，不可重试（CLAUDE.md 6.4） */
    PROVIDER_AUTH_FAILED(2001, "模型服务认证失败，请检查提供商配置"),

    /** 限流：provider 返回 429，可按 Retry-After 重试（CLAUDE.md 6.4） */
    PROVIDER_RATE_LIMITED(2002, "模型服务请求过于频繁，请稍后重试"),
